...
async function comDocumentoQueGuarda(page: import("@playwright/test").Page) {
  const secoes = new Map<
    string,
    { content: string; justification: string | null; status: string | null }
  >([
...
test.describe("troca de seção", () => {
  test("guarda o texto que ainda não tinha sido salvo", async ({ page }) => {
    await comSessao(page)
    await comProcessoEDocumento(page)
    const secoes = await comDocumentoQueGuarda(page)
    ...
    await page.getByRole("button", { name: /Seção 5 do ETP/ }).first().click()
    await page.waitForTimeout(400)
    ...
    await expect(page.locator("main textarea")).toHaveValue(
      "Requisitos da contratação, ainda em rascunho.",
    )
  })
})
